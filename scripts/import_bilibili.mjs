...
function getAdaptiveSummaryPrompt() {
  return [
    "请先判断视频类型，再只选择一个最合适的模板输出。不要把通用模板和类型模板叠加，避免重复和臃肿。",
    "",
    "所有类型都必须先输出：",
    "## 内容类型",
    ...
    "- 课程/讲座：`## 一句话总结`、`## 知识框架`、`## 关键概念`、`## 复盘问题`、`## 值得回看`。",
    "- 产品评测：`## 一句话总结`、`## 适合谁`、`## 优点`、`## 缺点/风险`、`## 选择建议`。",
    "- 观点/认知：`## 一句话总结`、`## 核心观点`、`## 支撑理由`、`## 对我的启发`、`## 可验证问题`。",
    "- 资讯解读：`## 一句话总结`、`## 发生了什么`、`## 为什么重要`、`## 可能影响`、`## 值得跟进`。",
    "- 案例复盘：`## 一句话总结`、`## 背景`、`## 关键动作`、`## 结果`、`## 可借鉴经验`。",
    "- 访谈/播客：`## 一句话总结`、`## 嘉宾观点`、`## 关键故事`、`## 延伸思考`。",
    "- 灵感素材：`## 一句话总结`、`## 可收藏信息`、`## 可能用途`、`## 后续整理建议`。",
    "- 其他：自己生成 3-5 个最合适的栏目。",
    "",
    "长度控制：全文尽量控制在 600-1000 字；每个栏目 3-5 条以内；不要输出时间轴，除非视频本身强依赖时间顺序。"
  ].join("\n");
}
