...
- 若设置里填写 Z.AI API Key，插件会把网页正文、图片文字、视频转写稿交给 GLM-5.1 生成结构化总结。
- 纯图片笔记会优先使用 GLM-5V-Turbo 做图文理解；失败时回退到本地 RapidOCR/Tesseract。
- B 站视频会优先使用视频自带字幕/自动字幕；没有字幕时才下载音频并用本地 Whisper 转写。
- 若没有总结 API Key，插件仍会保存网页能提取到的内容和本地转写稿，并生成基础摘要。
